...
window.CRMKanban = (() => {
  ...
  function criarColuna(colunaData) {
    ...
    // Adiciona cardsContainer ao DOM
    coluna.appendChild(cardsContainer);

        // O restante da função já foi adicionado acima (eventos, estatísticas, etc.)
      } // <-- Fecha a função criarColuna
    })(); // <-- Fecha o IIFE do módulo CRMKanban